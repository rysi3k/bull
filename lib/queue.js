...
Queue.prototype.removeHandler = function(name) {
    if (!this.handlers[name]) {
      return;
    }

    this.stopProcessing = this.stopProcessing || 0;
    this.stopProcessing++;
    if (!this.closing) {
      this.closing = 1; // specjalnie, by odrożnic true/false
    }
...
Queue.prototype.processJobs = function(index, resolve, job) {
  ...
  process.nextTick(function() {
    if (_this.stopProcessing) {
      _this.stopProcessing--;
      if (_this.closing === 1) {
        if (_this.stopProcessing === 0) {
          _this.closing = false;
        }
      }
      _this.processing[index] = null;
      resolve('stop processing');
    } else if (!_this.closing) {
...
Queue.prototype.processJob = function(job, notFetchNext = false) {
  var _this = this;
  var lockRenewId;
  ...
  function handleCompleted(result) {
    return job.moveToCompleted(result, undefined, notFetchNext).then(function(jobData) {
