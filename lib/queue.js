...
Queue.prototype.startMoveUnlockedJobsToWait = function() {
  clearInterval(this.moveUnlockedJobsToWaitInterval);
  // sprawdzamy niezgodnosc z true, bo potrafi sie nam gryźć z "1" od stopProcessing
  if (this.settings.stalledInterval > 0 && this.closing !== true) {
          ...
          if (jobId) {
            if (this.stopProcessing > 0) {
              return void await Promise.all([
                this.bclient.rpush(this.keys.wait, jobId),
                this.bclient.lrem(this.keys.active, 1, jobId),
              ]);
            }
